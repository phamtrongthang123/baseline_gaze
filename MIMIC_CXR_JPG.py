...
import os
class MIMIC_CXR_JPG():
    def __init__(self, path="./gcloud/mimic-cxr-jpg-2.0.0.physionet.org", init_now=False) -> None:
    ...
    def get_stats_overview(self,):
        ...
        patients = [patient for big_folder in big_folders for patient in big_folder.values() if type(patient) is dict]
        studies = [study for big_folder in big_folders for patient in big_folder.values() if type(patient) is dict for study in patient.values() if type(study) is dict]
        images = [img for big_folder in big_folders for patient in big_folder.values() if type(patient) is dict for study in patient.values() if type(study) is dict for img in study["imgs"]]
        num_big_folders = len(big_folders)
        num_patients = len(patients)
        num_studies = len(studies)
        num_images = len(images)
        number_overview = {"num_big_folders": num_big_folders, "num_patients": num_patients, "num_studies": num_studies, "num_images": num_images}
    ...
    def get_extremelar_sample(self):
        studies = [study for big_folder in self.data.values() for patient in big_folder.values() if type(patient) is dict for study in patient.values() if type(study) is dict]
        images_per_study = np.array([len(study["imgs"]) for study in studies])
    ...
    def get_row_from_image_ids(self, image_ids):
        ...
        """
        meta = pd.read_csv("./gcloud/mimic-cxr-jpg-2.0.0.physionet.org/mimic-cxr-2.0.0-metadata.csv")
    ...
    def pull_sample_from_dicom_id(self, dicom_id,saved_dir):
        ...
        image_path = os.path.join(self.path,'files',f'p{str(meta["subject_id"].unique()[0])[:2]}','p'+str(meta['subject_id'].unique()[0]), 's'+str(meta['study_id'].unique()[0]), str(meta['dicom_id'].unique()[0])+".jpg")
