...
for dicom_id, v in metadata.items():
    ...
    if len(v['reflacx_id']) != 1:
        # continue
        for transcript_path, gaze_path in zip(v['transcript_reflacx'], v['gaze_reflacx']):
            # read transcript
            ...
            for _, row in transcript.iterrows():
                word,timestamp_start_word,timestamp_end_word = row['word'], row['timestamp_start_word'], row['timestamp_end_word']
                if is_start:
...
get_stat = lambda x: '\n mean: ' + str(np.mean(x).round(decimals=2)) + ' std: ' + str(np.std(x).round(decimals=2)) + ' median: ' + str(np.median(x).round(decimals=2)) + '\n max: ' + str(np.max(x).round(decimals=2)) + ' min: ' + str(np.min(x).round(decimals=2))
# reflacx_sentence_length = [] 
# reflacx_word_length = []
# reflacx_full_text_length = []
# reflacx_num_sentences = []
...
axs[2, 0].set_title('reflacx gaze length per sentence' + get_stat(reflacx_gaze_length_per_sentence))
fig.tight_layout()
# plt.show()
...
axs[2, 0].set_title('egd gaze length per sentence' + get_stat(egd_gaze_length_per_sentence))
fig.tight_layout()
# plt.show()
