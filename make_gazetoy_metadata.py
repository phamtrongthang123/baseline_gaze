...
make_element = lambda patient_id, study_id, img_path_jpg, img_path_dcm, transcript_egd, gaze_egd, fixation_egd, reflacx_id, transcript_reflacx, gaze_reflacx, fixation_reflacx: {"patient_id": patient_id, "study_id": study_id, "img_path_jpg": img_path_jpg, "img_path_dcm": img_path_dcm, "transcript_egd": transcript_egd, "gaze_egd": gaze_egd,"fixation_egd":fixation_egd, "reflacx_id":[reflacx_id],"transcript_reflacx": [transcript_reflacx], "gaze_reflacx": [gaze_reflacx], "fixation_reflacx": [fixation_reflacx]}

make_image_path = lambda patient_id, study_id, dicom_id: ('data_here/mimic-cxr/2.0.0/files/p{}/p{}/s{}/{}.dcm'.format(patient_id[:2], patient_id, study_id, dicom_id), 'data_here/mimic-cxr-jpg-2.0.0/files/p{}/p{}/s{}/{}.jpg'.format(patient_id[:2], patient_id, study_id, dicom_id))
...
metadata_reflacx = [pd.read_csv(reflacx_maindir/"metadata_phase_{}.csv".format(i), dtype=str) for i in range(1, 4)]
...
for i,df in enumerate(metadata_reflacx):
    for index, row in df.iterrows():
        ...
        study_id = row['image'].replace('physionet.org/files/mimic-cxr/2.0.0/files/', '').split('/')[2][1:]
        ...
        fixation_egd = '' 
        reflacx_id = row['id']
        transcript_reflacx = f'{str(reflacx_maindir)}/{reflacx_id}/timestamps_transcription.csv'
        ...
        try:
            ...
            if dicom_id not in metadata_dict:
                metadata_dict[dicom_id] = make_element(subject_id, study_id, img_path_jpg, img_path_dcm, transcript_egd, gaze_egd, fixation_egd, reflacx_id, transcript_reflacx, gaze_reflacx, fixation_reflacx)
            else:
...
for index, row in metadata_egd.iterrows():
    ...
    transcript_egd = f'{str(egd_maindir)}/audio_segmentation_transcripts/{dicom_id}/transcript.json'
    ...
    fixation_egd =  os.path.join(f'{str(egd_maindir)}/fixations/{dicom_id}', f'fixations.csv')
