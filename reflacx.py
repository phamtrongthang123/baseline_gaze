...
class REFLACX:
    ...
    def pull_labels_from_dicom_id(self, dicom_id, saved_dir):
        ...
        os.system(
            "cp {} -rf {}".format(
                os.path.join(self.main_dir, id),
                os.path.join(saved_main_data, f"{dicom_id}_{id}"),
            )
        )
        os.system(
            "cp {} -rf {}".format(
                os.path.join(self.gaze_dir, id),
                os.path.join(saved_gaze_data, f"{dicom_id}_{id}"),
            )
        )
...
if __name__ == "__main__":
    ...
    metadata_phases = [
        pd.read_csv(reflacx.main_dir / "metadata_phase_{}.csv".format(i))
        for i in range(1, 4)
    ]
        ...
        for row in p.iterrows():
            image_path = (
                ...
                .replace(
                    ...
                    "./gcloud/mimic-cxr-jpg-2.0.0.physionet.org/",
                )
                .replace(".dcm", ".jpg")
            )
