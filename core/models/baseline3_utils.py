...
class ImageEncoder(nn.Module):
    def __init__(self, config):
        ...
        #     nn.Linear(self.img_encoder.fc.in_features, config["hidden_size"]//config['bottleneck']),
        #     nn.ReLU(),
        #     nn.Linear(config["hidden_size"]//config['bottleneck'], config["hidden_size"]),
        # )  # make it always to hidden size
        # We want to keep the most of image resnet features
        self.img_mapping_to_hidden = nn.Sequential(
...
class ImageFixationFuser(nn.Module):
    def __init__(self, config):
        ...
        self.query_embed = nn.Embedding(50, config["hidden_size"])
        self.transformer_decoder_layer_query = nn.TransformerDecoderLayer(
            config["hidden_size"],
            config["num_attention_heads"],
            dim_feedforward=config["intermediate_size"],
            dropout=config["hidden_dropout_prob"],
        ...
        self.norm_capout_query = nn.LayerNorm(config["hidden_size"])
        self.transformer_decoder_query = nn.TransformerDecoder(
            self.transformer_decoder_layer_query,
            config["num_hidden_layers"],
            norm=self.norm_capout_query,
        )
    ...
    def forward(self, img_features, fix_feature, length):
        ...
        fused_img_fix = self.transformer_decoder(fix_feature, img_features) # torch.Size([1, 400, 512])
        fused_img_fix = self.query_mlp(fused_img_fix)
        # fused_img_fix = einops.rearrange(fused_img_fix, 'b l d -> l b d') # [400, 1, 512]
        # make clone diff   
        # query = self.query_embed(torch.arange(0, length, device=fused_img_fix.device)) # [length, 512]
        # repeated_query = query.unsqueeze(0).repeat(fused_img_fix.shape[0], 1, 1) # [400, length, 512]
        # fused_img_fix = self.transformer_decoder_query(repeated_query, fused_img_fix) # [400, length, 512]
        # fused_img_fix = einops.rearrange(fused_img_fix, 'l b d -> b l d') # [length, 400, 512]     
        # fused_img_fix = self.query_mlp(fused_img_fix)

        fused_img_fix = einops.repeat(
            fused_img_fix, "b s d -> (l b) s d", l=length
        )  # torch.Size([3, 400, 512])
        query = self.query_embed(torch.arange(0, length, device=fused_img_fix.device)) # [length, 512]
        repeated_query = query.unsqueeze(1).repeat(1, fused_img_fix.shape[1], 1) # [length, 400, 512]
        # fused_img_fix = self.transformer_decoder_query(repeated_query, fused_img_fix) # [length, 400, 512]
        # fused_img_fix = einops.rearrange(fused_img_fix, 'l b d -> b l d') # [length, 400, 512]     
        # fused_img_fix = self.query_mlp(fused_img_fix)
        fused_img_fix = self.query_mlp(fused_img_fix+repeated_query)
        fused_img_fix = self.double_pe(fused_img_fix)
        return fused_img_fix
