...
def visualize_gaze_with_image_and_save(
    xs, ys, image, save_path, points=False, box=False, alpha=0.5
):
...
def resize_follow_img(
    v: torch.Tensor, img_old_size: int, img_new_size: int
) -> torch.Tensor:
...
def process_all(dicom_id):
    # dicom_id = '8e457921-bc1af8aa-a65073c1-aaac8247-c5ceb780'
    ...
    try:
        if (
            len(metadata[dicom_id]["fixation_reflacx"]) > 0
            and metadata[dicom_id]["fixation_reflacx"][0] != ""
        ):
            for i, (reflacx_id, f_path, g_path) in enumerate(
                zip(
                    metadata[dicom_id]["reflacx_id"],
                    metadata[dicom_id]["fixation_reflacx"],
                    metadata[dicom_id]["gaze_reflacx"],
                )
            ):
                fixation_reflacx = pd.read_csv(f_path)
                gaze_reflacx = pd.read_csv(g_path)
                new_gaze_reflacx = {"time": [], "x": [], "y": []}
                new_fixation_reflacx = {
                    "start_time": [],
                    "end_time": [],
                    "x": [],
                    "y": [],
                }
                nx, ny, mask = preprocess(
                    gaze_reflacx["x_position"].values, gaze_reflacx["y_position"].values
                )
                new_gaze_reflacx["x"] = vmap_resize_follow_img(
                    torch.tensor(nx + dw1), w, nw
                ).tolist()
                new_gaze_reflacx["y"] = vmap_resize_follow_img(
                    torch.tensor(ny + dh1), h, nh
                ).tolist()
                new_gaze_reflacx["time"] = (
                    gaze_reflacx["timestamp_sample"].values[mask].tolist()
                )
                nx, ny, mask = preprocess(
                    fixation_reflacx["x_position"].values,
                    fixation_reflacx["y_position"].values,
                )
                new_fixation_reflacx["x"] = vmap_resize_follow_img(
                    torch.tensor(nx + dw1), w, nw
                ).tolist()
                new_fixation_reflacx["y"] = vmap_resize_follow_img(
                    torch.tensor(ny + dh1), h, nh
                ).tolist()
                new_fixation_reflacx["start_time"] = (
                    fixation_reflacx["timestamp_start_fixation"].values[mask].tolist()
                )
                new_fixation_reflacx["end_time"] = (
                    fixation_reflacx["timestamp_end_fixation"].values[mask].tolist()
                )

                visualize_gaze_with_image_and_save(
                    new_fixation_reflacx["x"],
                    new_fixation_reflacx["y"],
                    img,
                    save_path=f"{vis_dir}/{dicom_id}-{reflacx_id}-reflacx.jpg",
                    points=True,
                    alpha=0.2,
                )
                with open(
                    f"{gaze_dir}/{dicom_id}-{reflacx_id}-reflacx-gaze.json", "w"
                ) as f:
                    json.dump(new_gaze_reflacx, f)
                with open(
                    f"{fix_dir}/{dicom_id}-{reflacx_id}-reflacx-fixation.json", "w"
                ) as f:
        ...
        if len(metadata[dicom_id]["fixation_egd"]) > 0:
            ...
            nx, ny, mask = preprocess(
                fixation_egd["FPOGX"].values, fixation_egd["FPOGY"].values
            )
            new_fixation_egd["x"] = vmap_resize_follow_img(
                torch.tensor(nx + dw1), w, nw
            ).tolist()
            new_fixation_egd["y"] = vmap_resize_follow_img(
                torch.tensor(ny + dh1), h, nh
            ).tolist()
            new_fixation_egd["start_time"] = fixation_egd["FPOGS"].values[mask].tolist()
            new_fixation_egd["end_time"] = (
                fixation_egd["FPOGS"].values + fixation_egd["FPOGD"].values
            )[mask].tolist()
            nx, ny, mask = preprocess(
                gaze_egd["X_ORIGINAL"].values, gaze_egd["Y_ORIGINAL"].values
            )
            new_gaze_egd["x"] = vmap_resize_follow_img(
                torch.tensor(nx + dw1), w, nw
            ).tolist()
            new_gaze_egd["y"] = vmap_resize_follow_img(
                torch.tensor(ny + dh1), h, nh
            ).tolist()
            new_gaze_egd["time"] = gaze_egd["Time (in secs)"].values[mask].tolist()
            visualize_gaze_with_image_and_save(
                new_fixation_egd["x"],
                new_fixation_egd["y"],
                img,
                save_path=f"{vis_dir}/{dicom_id}egd.jpg",
                points=True,
                alpha=0.2,
            )
...
def write_new_metadata_all(dicom_id):
    ...
    try:
        if (
            ...
            for i, (reflacx_id, f_path, g_path) in enumerate(
                ...
                if i == 0:
                    new_metadata[dicom_id]["fixation_reflacx"] = [
                        f"{fix_dir}/{dicom_id}-{reflacx_id}-reflacx-fixation.json"
                    ]
                    new_metadata[dicom_id]["gaze_reflacx"] = [
                        f"{gaze_dir}/{dicom_id}-{reflacx_id}-reflacx-gaze.json"
                    ]
                else:
                    new_metadata[dicom_id]["fixation_reflacx"].append(
                        f"{fix_dir}/{dicom_id}-{reflacx_id}-reflacx-fixation.json"
                    )
                    new_metadata[dicom_id]["gaze_reflacx"].append(
                        f"{gaze_dir}/{dicom_id}-{reflacx_id}-reflacx-gaze.json"
                    )

        if len(metadata[dicom_id]["fixation_egd"]) > 0:
            new_metadata[dicom_id][
                "fixation_egd"
            ] = f"{fix_dir}/{dicom_id}-egd-fixation.json"
...
if WRITE_META:
    for dicom_id in tqdm(dicom_ids):
        write_new_metadata_all(dicom_id)
