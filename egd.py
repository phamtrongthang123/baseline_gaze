...
class EGD:
    ...
    def pull_labels_from_dicom_id(self, dicom_id, saved_dir):
        ...
        audio_segmentation_transcripts = os.path.join(
            saved_dir, "audio_segmentation_transcripts"
        )
        inclusion_exclusion_criteria_outputs = os.path.join(
            saved_dir, "inclusion_exclusion_criteria_outputs"
        )
        os.makedirs(audio_segmentation_transcripts, exist_ok=True)
        os.makedirs(inclusion_exclusion_criteria_outputs, exist_ok=True)
        os.system(
            "cp {} -rf {}".format(
                os.path.join(self.path, "audio_segmentation_transcripts", dicom_id),
        ...
        chf = pd.read_csv(
            os.path.join(self.path, "inclusion_exclusion_criteria_outputs", "CHF.csv")
        )
        normals = pd.read_csv(
            os.path.join(
                self.path, "inclusion_exclusion_criteria_outputs", "normals.csv"
            )
        )
        pneumonia = pd.read_csv(
            os.path.join(
                self.path, "inclusion_exclusion_criteria_outputs", "pneumonia.csv"
            )
        )
...
if __name__ == "__main__":
    ...
    for row in master_csv.iterrows():
        image_path = (
            row[1]["path"]
            .replace("files/", "./gcloud/mimic-cxr-jpg-2.0.0.physionet.org/files/")
            .replace(".dcm", ".jpg")
        )
